...
function showInstructions() {
    ...
    <div id="hide-helper-instructions-popup" class="hide-helper-instructions-popup">
        ...
        <div class="hide-helper-instructions-content">
            <h2>核心功能：设置保留消息数量</h2>
            <p>
                要设置希望保留的最新消息数量，请在 <strong class="button-like">保存设置</strong> 按钮右侧的输入框中输入一个数字，我们称之为“X”。
            </p>
            <p>
            ...
            <h2>识别被隐藏的消息</h2>
            <p>
                当一条消息被成功隐藏后，其内容上方会显示一个 <span class="icon-example"><i class="fa-solid fa-ghost"></i></span> 幽灵图标。这个图标明确地表明了该条消息当前处于隐藏状态（也即不会发送给AI）。
            </p>
            ...
            <p>
                插件会智能地、持续地工作：
                <ul>
                    <li>它会自动维护设定的数字“X”值。无论是发送新消息、接收 AI 的回复，还是删除现有消息，插件都会动态调整，确保始终只有最新的 X 条消息正常发送，其余的保持隐藏。</li>
            ...
            <p>
                <strong>请注意：</strong> “当前隐藏设置”是指在当前选定模式（全局或聊天）下，当前正在聊天的角色卡或群聊使用隐藏设置值。
            </p>
