...
function showInstructions() {
    ...
    <div id="hide-helper-instructions-popup" class="hide-helper-instructions-popup">
        ...
        <div class="hide-helper-instructions-content">
            ...
            <p>
                点击 <strong class="button-like">[保存设置]</strong> 后，插件将仅显示最新的 X 条消息（楼），并自动隐藏所有在此之前的较早消息。
            </p>
            <p>
                <strong>例如：</strong>
                假设当前聊天窗口共有 10 条消息（从第 0 楼到第 9 楼）。
                <ul>
                    <li>您在输入框中输入数字 <code>4</code>。</li>
                    <li>点击 <strong class="button-like">[保存设置]</strong>。</li>
                    <li>结果：最新的 4 条消息（即第 6 楼到第 9 楼）将保持可见。</li>
            ...
            <h2>识别被隐藏的消息</h2>
            <p>
                当一条消息被成功隐藏后，其内容上方会显示一个 <span class="icon-example"><i class="fa-solid fa-ghost"></i></span> 幽灵图标。这个图标明确地指示了该条消息当前处于隐藏状态。
            </p>
            ...
            <p>
                插件会智能地、持续地工作：
                <ul>
                    <li>它会自动维护您设定的"X"值。无论您是发送新消息、接收 AI 的回复，还是删除现有消息，插件都会动态调整，确保始终只有最新的 X 条消息可见，其余的保持隐藏。</li>
            ...
            <h2>隐藏模式：全局与聊天</h2>
            <p>
                本插件提供了两种操作模式，以适应不同的使用场景：
                <ul>
                    <li><strong>全局模式 (Global Mode)：</strong>在此模式下，您设置的隐藏数值"X"将应用于<strong>所有</strong>的角色卡和群聊。这是一个统一的、全局性的设置。</li>
                    <li><strong>聊天模式 (Chat Mode)：</strong>在此模式下，隐藏设置将<strong>仅</strong>应用于当前您正在操作的角色卡或群聊。您可以为每一个角色卡或群聊设置并保存一个独立的、不同的隐藏数值"X"。</li>
                </ul>
            </p>
            <p>
                您可以通过点击 <strong class="button-like">[关闭]</strong> 按钮旁边的 <strong class="button-like">[全局模式 / 聊天模式]</strong> 切换按钮，在两种模式之间轻松切换。当前生效的模式会显示在按钮上。
            </p>
            <p>
                <strong>请注意：</strong> "当前隐藏设置"总是指您在当前选定模式（全局或聊天）下，为当前目标（所有聊天或特定聊天）所应用的设置。
            </p>

            <h2>启用与禁用插件</h2>
            <p>
                如果您需要暂时停用或重新启用本插件的功能，可以前往"酒馆"（Tavern）主界面的插件管理页面，在那里找到"隐藏助手"并进行启用或禁用操作。
            </p>

            <h2>问题与反馈</h2>
            <p>
                我们致力于不断改进插件。如果您在使用过程中遇到任何问题、发现任何 Bug，或者有任何功能上的建议，都非常欢迎随时向我们反馈！感谢您的支持！
            </p>
        </div>
    </div>`;
